...
object mcp_fab_daily_volume_5mif {
  def main(args: Array[String]): Unit = {
    ...
    val sourceDF: DataFrame = sparkHive.sql(
      """
        ...
        |	(
        |	--通过T01表获取车型6位码
        |		select werk, spj, knr, vzgi from cpy_fh01t01
        |		union all
        |		select werk, spj, knr, vzgi from meb_fh01t01
        |		union all
        |		select werk, spj, knr, vzgi from cpn_fh01t01
        |		union all
        |		select werk, spj, knr, vzgi from cph_fh01t01
        |		union all
        |		select werk, spj, knr, vzgi from cpc_fh01t01
