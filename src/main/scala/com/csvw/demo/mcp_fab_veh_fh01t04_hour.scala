...
object mcp_fab_veh_fh01t04_hour {
  def main(args: Array[String]): Unit = {
    ...
    sparkHive.sql(
      s"""
         |insert overwrite table mcp.mcp_fab_veh_fh01t04_hi partition (cal_date)
         |select distinct
         |	p1.vin,
         |	p1.werk,
         |	p1.spj,
         ...
         |			when a.status0 = 'R500' and a.werk = '78' AND a.fanlage2 like '%CP3%' and a.anlbgr3 = 'IR51' then 'PFA3 L1'
         |			when a.status0 = 'R500' and a.werk = '78' AND a.fanlage2 like '%CP3%' and a.anlbgr3 = 'IR52' then 'PFA3 L2'
         |		end factory
         |		,d.pr_nr
         |		,a.status0
         |		,a.z89x_flag
         ...
         |		,a.mdatumzeit
         |		,CASE
         |			WHEN a.werk = 'CS' or (a.werk='78' and a.status0 in ('Z900','V900') and d.pr_nr in ('SE3','SV6')) THEN from_unixtime(unix_timestamp(substr(mdatumzeit, 1, 19), 'yyyy-MM-dd HH:mm:ss')- 330 * 60, 'yyyy-MM-dd')
         |			WHEN a.werk = 'C6' AND substr(a.anlbgr3,-1,1) = 'H' or (a.werk='78' and a.status0 in ('Z900','V900') and d.pr_nr in ('SE2','SV3')) THEN from_unixtime(unix_timestamp(substr(mdatumzeit, 1, 19), 'yyyy-MM-dd HH:mm:ss')-6 * 60 * 60, 'yyyy-MM-dd')
         |			WHEN a.werk = 'C6' AND substr(a.anlbgr3,-1,1) IN ('J','K') THEN from_unixtime(unix_timestamp(substr(mdatumzeit, 1, 19), 'yyyy-MM-dd HH:mm:ss')-6 * 60 * 60, 'yyyy-MM-dd')
         |			WHEN (a.werk = 'C5' AND werk0 = 'C5') OR (a.werk = 'C2' AND status0 in ('Z900','V900') and d.pr_nr = 'SE3') THEN from_unixtime(unix_timestamp(substr(mdatumzeit, 1, 19), 'yyyy-MM-dd HH:mm:ss')-5 * 60 * 60, 'yyyy-MM-dd')
         |			WHEN a.werk = 'C2' AND status0 in ('Z900','V900') and d.pr_nr = 'SE1' THEN from_unixtime(unix_timestamp(substr(mdatumzeit, 1, 19), 'yyyy-MM-dd HH:mm:ss')-5 * 60 * 60, 'yyyy-MM-dd')
         |			WHEN a.werk = 'C2' THEN from_unixtime(unix_timestamp(substr(mdatumzeit, 1, 19), 'yyyy-MM-dd HH:mm:ss')-390 * 60, 'yyyy-MM-dd')
         |			WHEN a.werk = '78' AND a.fanlage2 like '%CP2%' THEN from_unixtime(unix_timestamp(substr(mdatumzeit, 1, 19), 'yyyy-MM-dd HH:mm:ss')-5 * 60 * 60, 'yyyy-MM-dd')
         |			WHEN a.werk = '78' AND a.fanlage2 like '%CP3%' THEN from_unixtime(unix_timestamp(substr(mdatumzeit, 1, 19), 'yyyy-MM-dd HH:mm:ss')-5 * 60 * 60, 'yyyy-MM-dd')
         |			WHEN a.werk = '78' AND fanlage2 like '%MEB%' THEN from_unixtime(unix_timestamp(substr(mdatumzeit, 1, 19), 'yyyy-MM-dd HH:mm:ss')-5 * 60 * 60, 'yyyy-MM-dd')
         |		END AS cal_date
         |		,b.vin
         |		,b.vzgi
         |		,b.modell
         ...
         |			WHERE substr(knr1,3,1) != '9' AND status0 IN ('A700','R700','L000','L800','M7X0','R100','L100','L500','M100','R500','V900','Z700','Z900','Z897','Z898') and werk = 'C5'
         |			UNION ALL
         |			-- 安亭
         |			SELECT knr1,werk,spj,kanr,fanlage2,status0,mdatum,mzeit,mdatumzeit,werk0,anlbgr3,geraetename3 FROM meb_fh01t04
         |			WHERE substr(knr1,3,1) != '9' AND status0 IN ('A700','R700','L000','L800','M7X0','R100','L100','L500','M100','R500','V900','Z700','Z900','Z897','Z898') and werk = '78'
         |			UNION ALL
         |			-- 南京
         |			SELECT knr1,werk,spj,kanr,fanlage2,status0,mdatum,mzeit,mdatumzeit,werk0,anlbgr3,geraetename3 FROM cpn_fh01t04
         |			WHERE substr(knr1,3,1) != '9' AND status0 IN ('A700','R700','L000','L800','M7X0','R100','L100','L500','M100','R500','V900','Z700','Z900','Z897','Z898') and werk = 'C2'
         |			UNION ALL
         |			-- 宁波
         |			SELECT knr1,werk,spj,kanr,fanlage2,status0,mdatum,mzeit,mdatumzeit,werk0,anlbgr3,geraetename3 FROM cph_fh01t04
         |			WHERE substr(knr1,3,1) != '9' AND status0 IN ('A700','R700','L000','L800','M7X0','R100','L100','L500','M100','R500','V900','Z700','Z900','Z897','Z898') and werk = 'C6'
         |			UNION ALL
         |			-- 长沙
         |			SELECT knr1,werk,spj,kanr,fanlage2,status0,mdatum,mzeit,mdatumzeit,werk0,anlbgr3,geraetename3 FROM cpc_fh01t04
         |			WHERE substr(knr1,3,1) != '9' AND status0 IN ('A700','R700','L000','L800','M7X0','R100','L100','L500','M100','R500','V900','Z700','Z900','Z897','Z898') and werk = 'CS'
         |		)t1
         |		left join
         |		(
         |		--过滤Z900中Z89X('Z897','Z898')的数据
         |			select werk,spj,kanr from meb_fh01t04
         |			where substr(knr1,3,1) != '9' AND status0 IN ('Z897','Z898') and werk='78'
         |			union all
         |			select werk,spj,kanr from cph_fh01t04
         |			where substr(knr1,3,1) != '9' AND status0 IN ('Z897','Z898') and werk='C6'
         |			union all
         |			select werk,spj,kanr from cpy_fh01t04
         |			where substr(knr1,3,1) != '9' AND status0 IN ('Z897','Z898') and werk='C5'
         |			union all
         |			select werk,spj,kanr from cpn_fh01t04
         |			where substr(knr1,3,1) != '9' AND status0 IN ('Z897','Z898') and werk='C2'
         |			union all
         |			select werk,spj,kanr from cpc_fh01t04
         |			where substr(knr1,3,1) != '9' AND status0 IN ('Z897','Z898') and werk='CS'
         ...
         |	-- 获取CPH1和CPC 中SKD车子ZP8点车辆KNR
         |	left join mcp.mcp_fab_veh_fh01t05_pr_hour d
         ...
         |	and p2.rn = 1
         |where p1.cal_date >= from_unixtime(unix_timestamp('${bizdate}','yyyyMMdd') - 15 * 24 * 60 * 60, 'yyyy-MM-dd')
         |"""
        .stripMargin)
