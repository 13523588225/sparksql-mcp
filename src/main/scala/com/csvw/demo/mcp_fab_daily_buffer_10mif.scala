  ...
  def main(args: Array[String]): Unit = {
    ...
    val sourceDF: DataFrame = sparkHive.sql(
      """
        ...
        |		max(case when status0 = 'M7X0' AND PLANT in ('CPA3','CPY') THEN NULL ELSE mdatumzeit end) M7X0_TIME,
        ...
        |(
        |	-- 剔除CPN转CPY和CPA3的车辆
        |	select 'CPN' plant,KANR from tmp where  plant in ('CPY','CPA3') union
        |	-- 剔除CPM转CPH1的车辆
        |	select 'CPM' plant,KANR from tmp where  plant = 'CPH1' union
        |	-- 剔除CPH1转CPH2的车辆
        |	select 'CPH1' plant,KANR from tmp where  plant = 'CPH2' union
        |	-- 剔除CPH2B2转CPH2B1的车辆
        |	select 'CPH2B2' plant,KANR from tmp where  plant = 'CPH2B1'
        |)b on a.plant = b.plant and a.KANR = b.KANR
