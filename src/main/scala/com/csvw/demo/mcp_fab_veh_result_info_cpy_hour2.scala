...
object mcp_fab_veh_result_info_cpy_hour2 {
  def main(args: Array[String]): Unit = {
    ...
    val kuduDF17: DataFrame = sparkKudu.read.format("org.apache.kudu.spark.kudu").option("kudu.table", cpy_fh01t04).option("kudu.master", kuduMaster).load()
    val kuduDF18: DataFrame = sparkKudu.read.format("org.apache.kudu.spark.kudu").option("kudu.table", cpy_fh01tq45).option("kudu.master", kuduMaster).load()
    val kuduDF19: DataFrame = sparkKudu.read.format("org.apache.kudu.spark.kudu").option("kudu.table", cpy_fh01tqc0)
      .option("kudu.master", kuduMaster).load().filter(s"capture_time >= '${bizdate}'")
    ...
    sparkHive.sql(
      s"""
         ...
         |	) t5
         |	on t4.werk = t5.werk and t4.spj = t5.spj and t4.kanr = t5.kanr0
         |	left join
