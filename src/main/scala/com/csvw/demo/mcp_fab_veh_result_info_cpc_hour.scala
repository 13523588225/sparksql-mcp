...
object mcp_fab_veh_result_info_cpc_hour {
  def main(args: Array[String]): Unit = {
    ...
    val kuduDF2: DataFrame = sparkKudu.read.format("org.apache.kudu.spark.kudu").option("kudu.table", cpc_fh01t04).option("kudu.master", kuduMaster).load()
    val kuduDF3: DataFrame = sparkKudu.read.format("org.apache.kudu.spark.kudu").option("kudu.table", cpc_fh01tq45).option("kudu.master", kuduMaster).load()
    val kuduDF4: DataFrame = sparkKudu.read.format("org.apache.kudu.spark.kudu").option("kudu.table", cpc_fh01tqc0).option("kudu.master", kuduMaster).load()
    val kuduDF5: DataFrame = sparkKudu.read.format("org.apache.kudu.spark.kudu").option("kudu.table", cpc_fh01tqc1).option("kudu.master", kuduMaster).load()
