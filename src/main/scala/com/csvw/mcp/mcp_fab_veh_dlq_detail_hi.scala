  ...
  def main(args: Array[String]): Unit = {
    ...
    sparkHive.sql(
      s"""
        ...
        |		a.capture_time,
        |		case
        |			when a.werk = 'CS' then from_unixtime(unix_timestamp(substr(a.capture_time,1,19),'yyyy-MM-dd HH:mm:ss') - 330 * 60, 'yyyy-MM-dd HH:mm:ss')
        |			when a.werk = 'C6' then from_unixtime(unix_timestamp(substr(a.capture_time,1,19),'yyyy-MM-dd HH:mm:ss') - 6 * 60 * 60, 'yyyy-MM-dd HH:mm:ss')
        |			when a.werk in ('C5','78') then from_unixtime(unix_timestamp(substr(a.capture_time,1,19),'yyyy-MM-dd HH:mm:ss') - 5 * 60 * 60, 'yyyy-MM-dd HH:mm:ss')
        |		end plant_time,
        |		a.series_code_6,
        |		a.series_name_6,
        |		a.werk,
